...
async def lifespan(app: FastAPI):
    print("[INFO] Building RAG engine (embeddings + vector store + Groq LLM)...")
    await asyncio.get_event_loop().run_in_executor(None, rag_engine.build)
    print("[INFO] JudicialGPT RAG engine is ready.")
    yield
    print("[INFO] Shutting down JudicialGPT.")
...
async def stream_rag_response(query: str) -> AsyncGenerator[str, None]:
    ...
    # Send completion signal and sources
    sources = [doc.metadata.get("source", "Unknown") for doc in context_docs]
    yield f"data: {json.dumps({'metadata': {'sources': sources}})}\n\n"
    yield "data: [DONE]\n\n"
