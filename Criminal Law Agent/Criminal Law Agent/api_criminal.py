...
NOTE: /query now takes multipart/form-data (query, session_id, optional
image) instead of a JSON body, so it can accept a file upload. Update any
existing client that was POSTing JSON to /query.
====================================================================
"""
...
async def query(
    ...
    **Example queries:**
    ...
    # filename — treat that the same as "no image attached".
    ...
    try:
        if has_image:
            ...
            if len(image_bytes) > MAX_IMAGE_BYTES:
                raise HTTPException(
                    ...
                    detail=f"Image too large ({len(image_bytes) // 1024} KB). "
                            f"Max allowed is {MAX_IMAGE_BYTES // (1024 * 1024)} MB.",
                )

            result = ag.ask_with_image(
                query=query,
            ...
            result = ag.ask(query, session_id=session_id)
