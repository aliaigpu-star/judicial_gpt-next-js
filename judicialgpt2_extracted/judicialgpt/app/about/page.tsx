      ...
      <section className="py-20 lg:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            <div className="hidden lg:block absolute left-[27px] top-4 bottom-4 w-px bg-gradient-to-b from-emerald-200 via-emerald-300 to-emerald-100" />
            <div className="space-y-6">
              {PROBLEM.points.map((point, i) => (
                <FadeIn key={i} delay={i * 0.06}>
                  <div className="flex gap-5 lg:gap-8 group">
                    <div className="relative shrink-0 w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-500/20 group-hover:scale-105 transition-transform">
      ...
      <section className="py-20 lg:py-24 bg-[#F2FBF6]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn className="text-center max-w-2xl mx-auto mb-14">
            ...
            <p className="text-emerald-700 font-medium text-lg">{AIMS.objective}</p>
          </FadeIn>

          <div className="grid lg:grid-cols-2 gap-8">
            <FadeIn>
              <div className="h-full rounded-3xl bg-white border border-emerald-100 p-8 shadow-sm">
                ...
                <ul className="space-y-4">
                  {AIMS.aims.map((aim) => (
                    <li key={aim} className="flex items-start gap-3">
                      <span className="mt-1 w-5 h-5 rounded-full bg-emerald-100 flex items-center justify-center shrink-0">
            ...
            <FadeIn delay={0.08}>
              <div className="h-full rounded-3xl bg-gradient-to-br from-emerald-600 to-teal-800 p-8 text-white shadow-xl shadow-emerald-900/20">
                <div className="flex items-center gap-3 mb-6">
                ...
                <ul className="space-y-4">
                  {AIMS.vision.map((item) => (
                    <li key={item} className="flex items-start gap-3">
                      <span className="mt-2 w-1.5 h-1.5 rounded-full bg-emerald-300 shrink-0" />
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {DATA_SOURCES.map((source, i) => (
              <FadeIn key={source.title} delay={i * 0.05}>
                <div
                  className={`h-full p-6 rounded-2xl border transition-all hover:shadow-lg hover:shadow-emerald-500/5 ${
                    i === 0 || i === 3
                      ? 'bg-emerald-50/60 border-emerald-200 lg:row-span-1'
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {TEAM.map((member, i) => (
              <FadeIn key={member.name} delay={i * 0.06}>
                <article className="group h-full rounded-2xl bg-white border border-slate-200 p-6 hover:border-emerald-200 hover:shadow-xl hover:shadow-emerald-500/8 transition-all duration-300">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-400 to-emerald-700 flex items-center justify-center text-white font-bold text-lg mb-4 shadow-md shadow-emerald-500/25 group-hover:scale-105 transition-transform">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-5 gap-12">
            ...
            <FadeIn delay={0.1} className="lg:col-span-2">
              <div className="h-full rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-white p-8">
                <h3 className="font-heading text-xl font-semibold text-slate-900 mb-4">Get in touch</h3>
                <div className="space-y-4">
                  <a
                    href={`mailto:${SITE.email}`}
                    className="flex items-center gap-3 text-slate-600 hover:text-emerald-700 transition-colors group"
                  >
                ...
                <Link
                  href="/contact"
                  className="mt-6 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-500 text-white text-sm font-bold hover:bg-emerald-600 transition-colors"
                >
                  Contact the team
                </Link>
              </div>
