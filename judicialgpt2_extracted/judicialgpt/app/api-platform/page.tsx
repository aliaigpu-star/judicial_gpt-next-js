    ...
    <SiteShell>
      ...
      <section className="py-20 lg:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
            {[
              ...
              <FadeIn key={item.label} delay={i * 0.05}>
                <div className="h-full p-6 rounded-2xl border border-slate-200 bg-[#F7FAF8] hover:border-emerald-200 transition-colors">
          ...
          <FadeIn delay={0.15}>
            <div className="rounded-2xl border border-emerald-100 bg-[#F2FBF6] p-6 md:p-8">
              <p className="text-[11px] font-bold uppercase tracking-wider text-emerald-700 mb-4">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {TECH_STACK.models.map((model) => (
                  <div
                    key={model}
                    className="flex items-start gap-2.5 text-sm text-slate-600 bg-white rounded-xl px-4 py-3 border border-emerald-100/80"
                  >
              ...
              <div className="rounded-2xl overflow-hidden border border-slate-800 bg-slate-950 shadow-2xl">
                ...
                <div className="p-5 overflow-x-auto">
                  <pre className="text-[13px] leading-relaxed font-mono">
                    <code>
                      <span className="text-emerald-400">{SAMPLE_REQUEST}</span>
                      {'\n\n'}
            ...
            <FadeIn delay={0.08} className="lg:col-span-3">
              <ul className="grid sm:grid-cols-2 gap-4">
                {SECURITY_HIGHLIGHTS.map((item, i) => (
                  <li
                    key={item}
                    className="flex items-start gap-3 p-4 rounded-xl border border-slate-200 bg-slate-50/50 hover:border-emerald-200 hover:bg-emerald-50/30 transition-colors"
                  >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {ENTERPRISE_FEATURES.map((feat, i) => (
              <FadeIn key={feat.title} delay={i * 0.05}>
                <div className="h-full p-6 rounded-2xl bg-white border border-emerald-100/80 hover:shadow-lg hover:shadow-emerald-500/5 hover:border-emerald-200 transition-all duration-300">
                  <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center mb-4 shadow-md shadow-emerald-500/20">
          ...
          <FadeIn delay={0.2} className="mt-12 text-center">
            <div className="inline-flex flex-col sm:flex-row items-center gap-3">
              <Link
                href="/contact"
                className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl bg-emerald-500 text-white font-bold shadow-md shadow-emerald-500/25 hover:bg-emerald-600 transition-colors"
              >
                Request Enterprise Demo <ArrowRight className="w-4 h-4" />
              </Link>
              <Link
                href="/signup"
                className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl border border-slate-200 bg-white text-slate-700 font-semibold hover:border-emerald-200 transition-colors"
              >
                Create Account
              </Link>
            </div>
